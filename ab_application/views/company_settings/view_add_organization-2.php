<div class="col-md-12 main-content-div">
    <div class="main-content">
        ...
        <div class="container tag-box tag-box-v3" style="margin-top: 0px; padding-bottom: 15px; padding-top: 50px;">
            ...
            <a href="<?php echo base_url() . "Con_configaration/view_xyz"?>"><button class="btn btn-u">Test</button></a>
                ...
                <div id="pr_li" class="col-sm-8 well-sm">
                    ...
                    if ($query) {
                        ...
                        foreach ($query->result() as $row) {
                            ...
                            $arr1[] = $row->employee_id;
                            ?>

                            <div class="row" style="width: 100%<?php echo $count ?>; border: 1px solid; padding: 5px; margin-bottom: 20px" >
                                <?php
                    ...
                    <?php } } ?>
                            ...
                            <div style="width: 750px; height: 105px; border: 1px solid; padding: 5px; margin: 20px" >

                                <?php                            
//                            foreach ($arr1 as $rroow) {
//                                print_r($rrow);
//                                exit();
                                $this->db->order_by("sequence", "asc");
                                $root_query = $this->db->get_where('main_organization_settings', array('hierarchy' => $row->employee_id));
                                echo $count2 = count($row->employee_id);
                                if ($root_query) {
                                    $arr2 = array();
                                    foreach ($root_query->result() as $rrow) {
//                                        $count = 400 / $count2;
                                        $arr2[] = $rrow->employee_id;
                                        echo $count = count($arr2);

//                                    print_r($arr);
                                        ?>
                                        <div style="float: left; width: 200px; border: 1px solid; padding: 5px; margin-right: 15px" >
...
//                                } 
                                    ...
                                    <?php
                                    foreach ($arr2 as $rroow) {
                                    $this->db->order_by("sequence", "asc");
                                            $sroot_query = $this->db->get_where('main_organization_settings', array('hierarchy' => $rroow));
                                            if ($sroot_query) {
                                                foreach ($sroot_query->result() as $srow) {
                                                    ?>
                                                    <div style="width: 450px; border: 1px solid; padding: 5px; margin-bottom: 20px" >
                                                        <?php
