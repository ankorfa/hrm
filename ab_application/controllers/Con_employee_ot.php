...
class Con_employee_ot extends CI_Controller {
    ...
    public function index() {
        ...
        $param['page_header'] = "Employee OT";
        $param['module_id']=$this->module_id;;
        
        if ($this->user_group == 11 || $this->user_group == 12) {
            $param['query'] = $this->db->get_where('main_employee_ot', array('company_id' => $this->company_id,'isactive' => 1));
            $param['employee_query'] = $this->db->get_where('main_employees', array('company_id' => $this->company_id,'isactive' => 1));
        } else {
            $param['query'] = $this->db->get_where('main_employee_ot', array('isactive' => 1));
            $param['employee_query'] = $this->db->get_where('main_employees', array('isactive' => 1));
        }
        //echo $this->db->last_query();
        
        $param['left_menu'] = 'sadmin/hrm_leftmenu.php';
        $param['content'] = 'attendance/view_EmployeeOt.php';
        $this->load->view('admin/home', $param);
    }
    
    
     public function save_Employee_OT() 
    {
        ...
        else 
        {
            ...
            $res=$this->Common_model->insert_data('main_employee_ot',$data);
     ...
     function ajax_edit_employee_ot() {
        $id = $this->uri->segment(3);
        $data = $this->Common_model->get_by_id_row('main_employee_ot', $id);
        echo json_encode($data);
    }
    
    public function update_Employee_OT() 
    {
        $this->form_validation->set_rules('employee_id', 'Employee', 'required',array('required'=> "Please the enter required field, for more Info : %s."));
        $this->form_validation->set_rules('contract_hour', 'Contract Hour', 'required',array('required'=> "Please the enter required field, for more Info : %s."));
        $this->form_validation->set_rules('max_ot_hour', 'Max OT Hour', 'required',array('required'=> "Please the enter required field, for more Info : %s."));
        ...
        else 
        {
            ...
            $res=$this->Common_model->update_data('main_employee_ot',$data,array('id' => $this->input->post('ot_employee_id')));
